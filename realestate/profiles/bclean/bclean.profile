 ...
 */
function bclean_form_install_configure_form_alter(&$form, $form_state) {

  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = '_bclean_form_submit';

}

/**
 * bclean submit callback
 */
function _bclean_form_submit($form, &$form_state) {

  $bclean_file = dirname(__FILE__) . '/bclean.sql';
  $success = _bclean_import_bclean($bclean_file);
  
  if (!$success) {
    return;
  };

  // New reset the values the user filled in during the intial site setup.
...
/**
 * Import the datbase file
 */
function _bclean_import_bclean($filename) {
    ...
    if (!file_exists($filename) || !($fp = fopen($filename, 'r'))) {
        drupal_set_message(t('Unable to open bclean file %filename.', array('%filename' => $filename)), '
error');
        return FALSE;
    };
    ...
    foreach (_bclean_get_tables() as $table) {
        db_drop_table(substr($table, strlen($db_prefix)));
    };

    // Lead data from database file.
    $success = TRUE;
    $query = '';
    $new_line = TRUE;
    ...
    while (!feof($fp)) {
        ...
        if ($data[$len - 1] == "\n") {
            if ($data[$len - 2] == ';') {
                ...
                $sql = Database::getConnection($options['target'])->prepare($query);
                if (!$sql->execute(array(), $options)) {
                    if ($verbose) {
                        $success = FALSE;
                    };
                };
    ...
    if (!$success) {
        drupal_set_message(t('Failed to import database from %filename.', array('%filename' => $filename)), 'error');
    };
